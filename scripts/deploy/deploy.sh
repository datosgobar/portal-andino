...
if [ -z $dev_domain ] ; then
    ...
    dev_certs_dir="/tmp/dev_certs"
    DEPLOY_SSL_CRT_PATH="$dev_certs_dir/andino.crt"
    DEPLOY_SSL_KEY_PATH="$dev_certs_dir/andino.key"
    ssh -t $DEPLOY_TARGET_USERNAME@$DEPLOY_TARGET_IP -p$DEPLOY_TARGET_SSH_PORT "mkdir $dev_certs_dir"
    ssh -t $DEPLOY_TARGET_USERNAME@$DEPLOY_TARGET_IP -p$DEPLOY_TARGET_SSH_PORT "openssl req -x509 -nodes -days 365 -newkey rsa:2048 -subj \"/CN=$dev_domain/O=My Company Name LTD./C=AR\" -keyout $DEPLOY_SSL_KEY_PATH -out $DEPLOY_SSL_CRT_PATH"
fi
